defmodule ClassEase.Accounts do
  ...
  """
  def get_user_by_oauth_provider(provider, uid) do
  ...
  @doc """
  Registers or authenticates a user via OAuth.
  """
  def oauth_register_user(%{provider: provider, uid: uid} = oauth_data) do
    case get_user_by_oauth_provider(provider, uid) do
      nil ->
        case get_user_by_email(oauth_data.email) do
          nil ->
            # Create new user
            %User{}
  ...
  """
  def update_user_email(_user, token) do
  ...
  """
  def update_subscription_tier(user, tier) when tier in ["free", "teacher", "school"] do
  ...
  @doc """
  Tracks user login by updating last_login_at.
  """
  def track_login(user) do
    changeset = Ecto.Changeset.change(user, %{last_login_at: DateTime.truncate(DateTime.utc_now(), :second)})
    Repo.update(changeset)
  end
